...
class PointTransactionController {
    ...
    public function create($starr_id, $points_change, $reason, $created_by) {
        // IMPORTANT: Ensure STARR_POINTS entry exists for this user before creating transaction
        // This prevents foreign key constraint violations
        $user_points = $this->starr_points_model->getByStarrId($starr_id);
        if (!$user_points) {
            // Create STARR_POINTS entry if it doesn't exist
            $this->starr_points_model->starr_id = $starr_id;
            $this->starr_points_model->total_points = 0;
            $this->starr_points_model->last_login_date = date('Y-m-d H:i:s');
            $this->starr_points_model->login_streak = 0;
            
            if (!$this->starr_points_model->createPoints()) {
                return [
                    'success' => false,
                    'message' => 'Error creating user points record'
                ];
            }
        }
