...
if (!isset($_SESSION['user_id'])) {
    echo "You must be logged in to view your reports.";
    exit;
}
$userId = $_SESSION['user_id'];
$adminId = $_SESSION['user_id']; // Admin is the logged-in user

$reportController = new ReportController();
$responseController = new ResponseController();

// Fetch all reports for the user (merge legacy session IDs if present)
$reports = [];
$error = null;

// Current user_id
$primaryReports = $reportController->getReportsByReporter($userId);

// Legacy/older session key sometimes used in past builds
$legacyId = $_SESSION['id'] ?? null;
if ($legacyId && $legacyId !== $userId) {
    $legacyReports = $reportController->getReportsByReporter($legacyId);
} else {
    $legacyReports = [];
}

// Merge unique by reportId
$byId = [];
foreach (array_merge($primaryReports, $legacyReports) as $rep) {
    $byId[$rep->getReportId()] = $rep;
}
$reports = array_values($byId);

// Sort desc by reportDate to keep UI behavior stable
usort($reports, function($a, $b) {
    return strcmp($b->getReportDate(), $a->getReportDate());
});

// Fallback: if still empty, fetch all reports and filter by reporterId
if (empty($reports)) {
    $all = $reportController->getAllReports(); // returns assoc arrays
    $rebuilt = [];
    foreach ($all as $row) {
        if ((string)$row['reporterId'] === (string)$userId) {
            $report = new Report();
    ...
    usort($rebuilt, function($a, $b) { return strcmp($b->getReportDate(), $a->getReportDate()); });
    $reports = $rebuilt;
}
