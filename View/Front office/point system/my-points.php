...
// TODO: replace with real session/user lookup once user entity is ready
// $starr_id = null;
// if (isset($_SESSION['starr_id'])) {
//     $starr_id = (int)$_SESSION['starr_id'];
// } elseif (isset($_SESSION['id'])) {
//     $starr_id = (int)$_SESSION['id'];
// } elseif (isset($_GET['starr_id'])) {
//     $starr_id = (int)$_GET['starr_id'];
// }

// Get user ID from session
if (!isset($_SESSION['user_id'])) {
    die('<div class="alert alert-danger">You must be logged in to view your points.</div>');
}
$starr_id = $_SESSION['user_id'];

// Get user's points info
$user_points = $points_controller->getById($starr_id);
$user_data = $user_points['success'] ? $user_points['data'] : null;

// If no STARR_POINTS entry exists, create one for this user
if (!$user_data) {
    // Try to create a new STARR_POINTS entry for this user
    $create_result = $points_controller->create(
        $starr_id,
        0,  // total_points
        date('Y-m-d H:i:s'),  // last_login_date
        0   // login_streak
    );
...
if (!$user_data) {
    die('<div class="alert alert-danger">Unable to retrieve or create points record.</div>');
}
