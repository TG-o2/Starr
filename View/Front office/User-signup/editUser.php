...
$userId      = $_SESSION['user_id'];
$username    = $_SESSION['username'] ?? '';  // Add username to preserve it
...
// Default avatar path if none exists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($newUsername !== $username && !empty($newUsername)) {
    ...
    if (empty($errors)) {
        ...
        // Create User object with all parameters in correct order
        // Constructor: __construct($user_id, $password, $fname, $lname, $DOB, $profilePicture, $description, $username, $email, $role, $avatar, ...)
        $user = new User(
            $userId,                    // user_id
            ...
            !empty($newUsername) ? $newUsername : $username,  // username (can be edited now)
            $newEmail,                  // email
            $role,                      // role
            $newAvatar                  // avatar
        );
        ...
        $_SESSION['username'] = !empty($newUsername) ? $newUsername : $username;
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="text" name="username" class="form-control form-control-lg" value="<?= htmlspecialchars($username) ?>">
                                </div>
